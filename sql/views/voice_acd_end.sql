-- voice_acd_end
-- Deduplicated "latest state" view over the append-only voice_acd_end base table, enriched with
-- agent/queue names joined from the mapper reference views.

CREATE OR REPLACE VIEW "genesys_streaming_lakehouse_curated"."voice_acd_end" AS
WITH base AS (
  SELECT
    *,
    COALESCE(
  ...
  FROM "genesys_streaming_lakehouse"."voice_acd_end"
),
ranked AS (
  SELECT
    b.*,
...
SELECT
  ...
  d.event_id, d.event_time, d.ingest_time,
  coalesce(d.conversation_id, d.interaction_id) AS conversation_id,
  d.interaction_id, d.participant_id,
  ...
  d.ani, d.dnis,
  d.queue_id, q.name AS queue_name,
  d.division_id,
  d.acd_outcome, d.used_routing, d.routing_priority, d.connected_duration_ms, d.utilization_label,
  d.answered_user_id, u.name AS answered_user_name,
  d.conversation_external_contact_ids, d.requested_routings, d.flow_type,
  d.raw_event,
  d.opco, d.dt
FROM dedup d
LEFT JOIN "genesys_streaming_lakehouse_curated"."voice_mapper_users_current" u
  ON d.answered_user_id = u.id
