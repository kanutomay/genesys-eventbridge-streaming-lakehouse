-- voice_flow_end
-- Deduplicated "latest state" view over the append-only voice_flow_end base table.

CREATE OR REPLACE VIEW "genesys_streaming_lakehouse_curated"."voice_flow_end" AS
WITH base AS (
  SELECT
    *,
    COALESCE(
  ...
  FROM "genesys_streaming_lakehouse"."voice_flow_end"
),
ranked AS (
  SELECT
    b.*,
...
SELECT
  ...
  flow_type, flow_id, division_id, flow_version,
  connected_duration_ms, conversation_external_contact_ids,
  exit_reason, transfer_type, ani, dnis,
  raw_event,
  opco, dt
FROM ranked
WHERE rn = 1;
